...
**Algorithms:** contour generation via marching squares / OpenCV `findContours` on a quantized elevation raster; catchment delineation via D8 flow-direction + flow accumulation, tracing upstream contributing cells from the candidate point (pysheds/richdem); runoff via SCS Curve Number (primary) with a runoff-coefficient fallback (Rainfall × Catchment Area × Coefficient) when land/soil data is unavailable; pond sizing by targeting each candidate depth's own real terrain-holding capacity (flood-fill), back-calculating surface area within a practical depth range, checked against available land (see docs/DECISIONS.md D-007).
