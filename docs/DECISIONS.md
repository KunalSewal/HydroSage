...
## D-011: `contour_map` field name, and never fail a request over a rainfall outage

Date: 2026-09-01
Status: Accepted

**Context:** Preparing the Phase 1 submission surfaced two defects in the graded endpoint. First, the submission specification requires the uploaded file arrive under the multipart form field name `contour_map`; `POST /analyzeContour` took it as `file`, so a grader's request would have been rejected with a 422 before any analysis ran. Second, a failure from the Open-Meteo rainfall archive propagated out of `services/recommendation.py` as an unhandled exception, which FastAPI turns into a 500 — meaning a rate-limited or firewalled network made the endpoint return *nothing at all*, including the catchment analysis that is the actual deliverable. This was observed live three separate times during development when Open-Meteo returned 429. The deployment target is a lab VM whose outbound internet access is not guaranteed, which turns an intermittent annoyance into a likely total failure.

**Decision:** Rename the endpoint parameter to `contour_map` (FastAPI derives the form field name from the parameter name), and update the frontend caller in lockstep. Make a rainfall failure degrade rather than raise, mirroring the pattern `_get_available_land_hectares` already used for Overpass: `average_annual_rainfall_mm`, `runoff_volume_m3` and `runoff_coefficient` become `None`, and pond options are sized from terrain capacity alone. `size_pond_options` and `capture_ratio` accept `annual_runoff_m3: float | None`, where `None` means *unknown* (fall back to the terrain bound, the pre-D-010 behaviour) as distinct from zero. Both external-client timeouts move into settings so a firewalled deployment can turn them down instead of spending 40 seconds per request waiting to degrade.
